...
import { computeCheckInStatus } from "@/lib/attendanceLate";
...
export const POST = requireRole(["superadmin", "admin", "teacher"], async (req, _ctx, session) => {
  try {
    ...
    const setFields = { [field]: new Date(), source: source || "manual" };
    const setOnInsert = {};

    // Late Attendance Buffer Time System: only the check-in punch is
    // evaluated against the school's shift start + grace period.
    if (action === "check-in") {
      const settings = await SchoolSettings.findOne({ key: "school-settings" });
      const { status, lateMinutes } = computeCheckInStatus(setFields.checkInAt, settings);
      setFields.status = status;
      setFields.lateMinutes = status === "late" ? lateMinutes : null;
    } else {

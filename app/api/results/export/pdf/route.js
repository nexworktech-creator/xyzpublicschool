...
// Draws the school logo (if one was fetched) as a small square at the top
// of the header, so every layout consistently shows branding rather than
// text alone. `school.logoBuffer` is populated once, up front, in the GET
// handler below (PDFKit needs raw image bytes, not a remote URL).
function drawLogo(doc, school, x, y, size) {
  if (!school.logoBuffer) return;
  try {
    doc.image(school.logoBuffer, x, y, { fit: [size, size], align: "center", valign: "center" });
  } catch (err) {
    console.error("[results/export/pdf] logo draw failed", err);
  }
}
...
const HEADER_DRAWERS = {
  bordered(doc, school, style) {
    ...
    drawLogo(doc, school, PAGE_LEFT + 10, 50, 56);
  ...
  "double-border"(doc, school, style) {
    ...
    drawLogo(doc, school, PAGE_LEFT + 14, 54, 56);
  ...
  band(doc, school, style) {
    ...
    drawLogo(doc, school, PAGE_LEFT + 4, 12, 54);
  ...
  plain(doc, school, style) {
    drawLogo(doc, school, PAGE_LEFT, 40, 44);
...
function drawOfficialMarksheet(doc, { result, school, cfg, attendancePct }) {
  ...
  drawLogo(doc, school, PAGE_LEFT, 38, 42);
...
export const GET = requireRole(["superadmin", "admin", "teacher", "principal"], async (req) => {
  ...
  // PDFKit can only embed raw image bytes, not a remote URL, so the logo
  // (hosted on Cloudinary) has to be fetched into a Buffer up front. This
  // was the missing piece that made the logo never actually appear on
  // printed/downloaded report cards even though a URL was being saved.
  if (school.logoUrl) {
    try {
      const logoRes = await fetch(school.logoUrl);
      if (logoRes.ok) {
        school.logoBuffer = Buffer.from(await logoRes.arrayBuffer());
      }
    } catch (err) {
      console.error("[results/export/pdf] could not fetch school logo", err);
    }
  }
  const cfg = {

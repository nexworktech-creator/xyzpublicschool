...
export const POST = requireRole(["superadmin", "admin", "teacher"], async (req, _ctx, session) => {
  try {
    ...
    if (id) {
      // Cycle is "completed" once every student has been marked with a
      // final status (complete/incomplete/absent) — i.e. nobody left "pending".
      const status = entries?.every((e) => e.status && e.status !== "pending") ? "completed" : "in-progress";

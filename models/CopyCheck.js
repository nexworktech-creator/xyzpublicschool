...
const CopyCheckEntrySchema = new mongoose.Schema(
  {
    ...
    // Per-student progress on this checking cycle. "pending" = not looked at
    // yet, "complete" = copy checked and done, "incomplete" = copy was seen
    // but is missing pages/work/not finished, "absent" = student didn't
    // submit a copy because they were absent that day.
    status: { type: String, enum: ["pending", "complete", "incomplete", "absent"], default: "pending" },
    checkedOn: { type: Date }, // set/refreshed whenever status is last changed
